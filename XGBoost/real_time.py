...
xgb_model = joblib.load('xgboost_model.pkl')  # Load your trained XGBoost model
...
def predict_sign(sequence):
    # Flatten the sequence and convert it into a 2D array
    sequence = np.array(sequence).reshape(1, -1)

    # Make a prediction using the XGBoost model
    predicted_class_index = int(xgb_model.predict(sequence)[0])  # Convert to integer
    print(predicted_class_index)

    # Ensure that predicted_class_index is within the bounds of CLASSES_LIST
    if 0 <= predicted_class_index < len(CLASSES_LIST):
        predicted_class = CLASSES_LIST[predicted_class_index]
    else:
        predicted_class = "Invalid Class"  # Handle cases where the index is out of bounds
